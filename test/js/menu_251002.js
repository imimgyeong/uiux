...
$(document).ready(function(){
    /******************************************** 
        * 기준 1024px이하면 모바일 / 1025px이상이면  pc
        * 브라우저의 넓이에 따라서 지금 pc인지 모바일인지 구분
    *********************************************/ 
    ...
    $(window).resize(function(){//브라우저가 리사이즈될 때마다 1번실행 
        size_chk()//함수 호출
    })
    ...
    /******************************************** 
        ...
        제한 조건
        1. li중에서 오버한 한개만 over클래스 들어감
        2. 메뉴에서 벗어나면 over가 사라짐
    *********************************************/ 
    ...
    /******************************************** 
        ...
        header에 menu_over
        제한 조건
        -오버해서 생상된 흰 배경안에서는 over를 유지할것 아웃안됨
    *********************************************/
    ...
    $('header .util .lang .lang_open').on('focusin', function(){
        /* 키보드 tap키로 이동할때 header에 focusout으로 주면 메뉴 이동할때마다 아웃됨
           메뉴 다음에 나오는 button이나 a한테 focus가 가면 메뉴를 닫는것으로 함 */
        $('header').removeClass('menu_over')
    })

    /*****************************************************************  
        * 모바일에서 1차메뉴를 클릭하면 2차메뉴 열기
        * 메뉴가 열려있으면 나 자신을 닫고
        * 매뉴가 닫혀있으면 열려있는 다른 메뉴는 닫고 나는 열기
        * 클릭햇을때 메뉴가 열렸는지 닫혔는지 판단 1차메뉴 li에 open여부 따짐
    *******************************************************************/
    ...
    /*****************************************************
        header .gnb .gnb_open클릭 header에 menu_open추가
        header .gnb .gnb_close클릭 header에 menu_open삭제
    ******************************************************/
